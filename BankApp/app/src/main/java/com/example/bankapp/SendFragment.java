...
public class SendFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        arusertrans.clear();
        Cursor result = mydb.trans_Send(getaccountno);

        while (result.moveToNext()){
            int id = result.getInt(0);
            int amount= result.getInt(1);
            int user= result.getInt(3);
            userTransaction userta = new userTransaction(id,amount,user);
            arusertrans.add(userta);
        }
        sendlist = view.findViewById(R.id.sendlistview);
        SenderCustomAdapter senderCustomAdapter = new SenderCustomAdapter(getActivity(),arusertrans);
        sendlist.setAdapter(senderCustomAdapter);
        return view;
    }
}